...
class AlamedaUSDFetcher(Fetcher):
    def child_init(self):
        print(f"Initializing Alameda USDFetcher for {self.site['subdomain']}")
    ...
    def fetch_events(self):
        ...
        minutes_dir = f"{self.storage_dir}/{self.subdomain}/pdfs/BoardOfEducation"
        agendas_dir = (
            f"{self.storage_dir}/{self.subdomain}/_agendas/pdfs/BoardOfEducation"
        )
        ...
        for meeting in os.listdir(input_dir):
            ...
            if self.check_if_exists("BoardOfEducation", date_string, kind):
